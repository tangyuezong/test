...
require_once dirname(__DIR__) . '/ShortMsg/api_sdk/vendor/autoload.php';
...
class DaYuShortMsg
{
    ...
    public static function getAcsClient() {
        ...
        $accessKeyId = "LTAIYZWGezEBHL02"; // AccessKeyId

        $accessKeySecret = "BtSjp3VXYWD0Wxc2KUFKrxWnKh65lJ"; // AccessKeySecret
     ...
     * @return stdClass
     */
    public static function sendSms($phone, $template, $msgparam) {
        ...
        $request->setSignName("浙江千生佑信息科技");

        // 必填，设置模板CODE，应严格按"模板CODE"填写, 请参考: https://dysms.console.aliyun.com/dysms.htm#/develop/template
        $request->setTemplateCode($template);

        // 可选，设置模板参数, 假如模板中存在变量需要替换则为必填项
        $request->setTemplateParam(json_encode($msgparam, JSON_UNESCAPED_UNICODE));
...
// $response = DaYuShortMsg::sendSms();
// echo "发送短信(sendSms)接口返回的结果:\n";
// print_r($response);

// sleep(2);

// $response = DaYuShortMsg::sendBatchSms();
// echo "批量发送短信(sendBatchSms)接口返回的结果:\n";
// print_r($response);

// sleep(2);

// $response = DaYuShortMsg::querySendDetails();
